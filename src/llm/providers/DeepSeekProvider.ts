...
export class DeepSeekProvider implements LLMProvider {
  ...
  async chat(params: {
    ...
    if (!this.apiKey) {
      throw new Error("DeepSeekProvider: DEEPSEEK_API_KEY is not set");
    }
    ...
    if (!response.ok) {
      ...
      throw new Error(`DeepSeekProvider API error (${response.status}): ${errorText}`);
    }
    ...
    if (!data.choices || data.choices.length === 0) {
      throw new Error("DeepSeekProvider: No choices returned in response");
    }
