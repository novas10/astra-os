/**
 * AstraOS — GroqProvider.ts
 * Groq LLM provider (llama-3.3-70b-versatile, mixtral-8x7b-32768, gemma2-9b-it)
...
export class GroqProvider implements LLMProvider {
  ...
  async chat(params: {
    ...
    if (!response.ok) {
      ...
      throw new Error(`GroqProvider API error (${response.status}): ${errorText}`);
    }
    ...
    if (!data.choices || data.choices.length === 0) {
      throw new Error("GroqProvider: No choices returned in response");
    }
